...
setup_credentials() {
    ...
    # check_env() calls this when .env EXISTS but has empty credentials, and this
    # script runs under `set -o noclobber` — so a plain `>` here aborts the whole
    # script (errexit) immediately after the user has typed their IB password.
    # Back the old file up first: it is hand-editable by design (see the header
    # below), so regenerating would otherwise silently discard those edits.
    if [ -f "$BUILDDIR/.env" ]; then
        local backup
        backup="$BUILDDIR/.env.bak_$(date +%Y-%m-%d_%H-%M-%S)"
        cp "$BUILDDIR/.env" "$backup"
        chmod 600 "$backup" 2>/dev/null || true
        echo ""
        echo "Existing .env backed up to $(basename "$backup")"
        echo "  Regenerating replaces hand-edited settings — merge anything custom back."
    fi
    # Create empty and lock down BEFORE the password is written, so it never
    # exists on disk world-readable. `:>|` overrides noclobber; the heredoc then
    # appends (>> is unaffected by noclobber).
    : >| "$BUILDDIR/.env"
    chmod 600 "$BUILDDIR/.env"
    cat >> "$BUILDDIR/.env" <<EOF
# MMR Configuration — generated by docker.sh
# Edit this file to change credentials or settings.
# Mode 600 deliberately — this file holds the IB password in plaintext.
